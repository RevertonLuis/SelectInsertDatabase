...
# Some glbal parameters
table_title = "SIPREC PCJ - ESTIMATIVA DE PRECIPITAÇÃO (mm): %s (BRT)"
...
class WriteXLS():
    ...
    def __init__(self, file_name, date, boundaries, table_names):
        ...
        self.dates.sort()
        self.date = date
        self.table_title = table_title % (self.date.strftime("%d-%m-%Y %H:00"))
    ...
    def write_table(self):
        ...
        date = self.dates[len(self.dates) - 1]
        date_acc = date.replace(hour=hour) - datetime.timedelta(hours=3)
